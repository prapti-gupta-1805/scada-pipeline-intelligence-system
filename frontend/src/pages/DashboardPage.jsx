...
// TODO: Expected backend endpoint for telemetry health trend series: /api/v1/health-series
// TODO: Expected backend endpoint for recent predictions log: /api/v1/predictions/recent
// TODO: Expected backend endpoint / fields for operational health and risk exposure metrics
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (healthData?.series) {
          setHealthSeries(healthData.series);
        }
        if (healthData?.recentPredictions) {
          setRecentPredictions(healthData.recentPredictions);
        }
      } catch (error) {
        console.error(error);
      } finally {
  ...
  const modelCoverageText = health?.models
    ? `${Object.values(health.models).filter(Boolean).length}/${Object.keys(health.models).length}`
    : loading
      ? '...'
      : '—';

  const operationalHealthText = health?.operational_health
  ...
  const riskExposureText = health?.risk_exposure || (loading ? '...' : '—');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <StatCard title="Model coverage" value={modelCoverageText} detail="Fault, anomaly, and maintenance models active" accent="amber" />
        <StatCard title="Insights" value={metadata ? `${Object.keys(metadata).length} modules` : loading ? '...' : '—'} detail="Connected to backend metadata" accent="rose" />
      </div>

      <div className={healthSeries.length > 0 ? "grid gap-6 lg:grid-cols-[1.2fr_0.8fr]" : "grid gap-6"}>
          ...
          <div className="mt-5 overflow-hidden rounded-2xl border border-slate-800">
            <table className="min-w-full divide-y divide-slate-800 text-sm">
              ...
              <tbody className="divide-y divide-slate-800 bg-slate-900/60">
                {recentPredictions.map((row, index) => (
                  <tr key={row.id || index}>
